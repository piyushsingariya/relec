...
func TestStatementsConstructor(t *testing.T) {
	cases := []struct {
		name         string
		in           *Statements
		add          []Statement
	...
	}{
		{
			...
			in:           NewSingleStatement(Statement{Sid: "1", Effect: EffectAllow}),
		...
		{
			name:         "SliceStatement",
			in:           NewStatements([]Statement{{Sid: "1", Effect: EffectAllow}, {Sid: "2", Effect: EffectDeny}}...),
...
func TestDisallowUnknownFields(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			wantErr: `Statements is not a slice of statements: json: unknown field "NewField"`,
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestStatementsUnmarshalJSON(t *testing.T) {
	cases := []struct {
		name         string
		in           string
		want         []Statement
		...
		},
		{
			name:    "BooleanJSON",
			in:      `true`,
			wantErr: ErrorInvalidStatements,
		},
		{
			name:    "BadJSON",
			in:      `{`,
		...
		},
		{
			...
			wantErr:      `Statements is not a slice of statements: json: unknown field "NotAField"`,
			wantSingular: false,
		},
		{
			...
			wantErr:      `Statements must be a single Statement or a slice of Statements: json: unknown field "NotAField"`,
			wantSingular: false,
		},
	}
	for _, tc := range cases {
...
func TestStatementsMarshalJSON(t *testing.T) {
	cases := []struct {
		name         string
		in           *Statements
		want         string
		...
		{
			name: "SingleStatement",
			in: NewSingleStatement(Statement{
				Effect:   EffectAllow,
				Action:   NewStringOrSlice(true, "s3:GetObject"),
				Resource: NewStringOrSlice(true, "arn:aws:s3:::my_corporate_bucket/exampleobject.png"),
			}),
