...
// CTE statement
ctes: WITH namedQuery (COMMA namedQuery)*;
...
alterStmt:
	ALTER TABLE tableIdentifier clusterClause? alterTableClause (
		COMMA alterTableClause
	)* # AlterTableStmt;

alterTableClause:
	ADD COLUMN (IF NOT EXISTS)? tableColumnDfnt (
		AFTER nestedIdentifier
	)? # AlterTableClauseAddColumn
	| ADD INDEX (IF NOT EXISTS)? tableIndexDfnt (
		AFTER nestedIdentifier
	)? # AlterTableClauseAddIndex
	| ADD PROJECTION (IF NOT EXISTS)? tableProjectionDfnt (
		AFTER nestedIdentifier
	)?													# AlterTableClauseAddProjection
	| ATTACH partitionClause (FROM tableIdentifier)?	# AlterTableClauseAttach
	| CLEAR COLUMN (IF EXISTS)? nestedIdentifier (
		IN partitionClause
	)? # AlterTableClauseClearColumn
	| CLEAR INDEX (IF EXISTS)? nestedIdentifier (
		IN partitionClause
	)? # AlterTableClauseClearIndex
	| CLEAR PROJECTION (IF EXISTS)? nestedIdentifier (
		IN partitionClause
	)?																# AlterTableClauseClearProjection
	...
	| MATERIALIZE INDEX (IF EXISTS)? nestedIdentifier (
		IN partitionClause
	)? # AlterTableClauseMaterializeIndex
	| MATERIALIZE PROJECTION (IF EXISTS)? nestedIdentifier (
		IN partitionClause
	)?																		# AlterTableClauseMaterializeProjection
	| MODIFY COLUMN (IF EXISTS)? nestedIdentifier codecExpr					# AlterTableClauseModifyCodec
	| MODIFY COLUMN (IF EXISTS)? nestedIdentifier COMMENT STRING_LITERAL	#
		AlterTableClauseModifyComment
	| MODIFY COLUMN (IF EXISTS)? nestedIdentifier REMOVE tableColumnPropertyType #
		AlterTableClauseModifyRemove
...
createStmt: (ATTACH | CREATE) DATABASE (IF NOT EXISTS)? databaseIdentifier clusterClause? engineExpr
		? # CreateDatabaseStmt
	| (ATTACH | CREATE (OR REPLACE)? | REPLACE) DICTIONARY (
		IF NOT EXISTS
	)? tableIdentifier uuidClause? clusterClause? dictionarySchemaClause dictionaryEngineClause #
	...
	| (ATTACH | CREATE) MATERIALIZED VIEW (IF NOT EXISTS)? tableIdentifier uuidClause? clusterClause
		? tableSchemaClause? (
		destinationClause
		| engineClause POPULATE?
	) subqueryClause # CreateMaterializedViewStmt
	| (ATTACH | CREATE (OR REPLACE)? | REPLACE) TEMPORARY? TABLE (
		IF NOT EXISTS
	)? tableIdentifier uuidClause? clusterClause? tableSchemaClause? engineClause? subqueryClause? #
		CreateTableStmt
	| (ATTACH | CREATE) (OR REPLACE)? VIEW (IF NOT EXISTS)? tableIdentifier uuidClause?
		clusterClause? tableSchemaClause? subqueryClause # CreateViewStmt;
...
dictionaryAttrDfnt
	...
	identifier columnTypeExpr (
		{!$attrs.Has("default")}? DEFAULT literal {$attrs.Insert("default");}
		| {!$attrs.Has("expression")}? EXPRESSION columnExpr {$attrs.Insert("expression");}
		| {!$attrs.Has("hierarchical")}? HIERARCHICAL {$attrs.Insert("hierarchical");}
		| {!$attrs.Has("injective")}? INJECTIVE {$attrs.Insert("injective");}
		| {!$attrs.Has("is_object_id")}? IS_OBJECT_ID {$attrs.Insert("is_object_id");}
	)*;
dictionaryEngineClause
	locals[string clauses]:
	dictionaryPrimaryKeyClause? (
		{!$clauses.Has("source")}? sourceClause {$clauses.Insert("source");}
		| {!$clauses.Has("lifetime")}? lifetimeClause {$clauses.Insert("lifetime");}
		| {!$clauses.Has("layout")}? layoutClause {$clauses.Insert("layout");}
		| {!$clauses.Has("range")}? rangeClause {$clauses.Insert("range");}
		| {!$clauses.Has("settings")}? dictionarySettingsClause {$clauses.Insert("settings");}
	)*;
...
engineClause
	locals[string clauses]:
	engineExpr (
		{!$clauses.Has("orderByClause")}? orderByClause {$clauses.Insert("orderByClause");}
		| {!$clauses.Has("partitionByClause")}? partitionByClause {$clauses.Insert("partitionByClause");
			}
		| {!$clauses.Has("primaryKeyClause")}? primaryKeyClause {$clauses.Insert("primaryKeyClause");
			}
		| {!$clauses.Has("sampleByClause")}? sampleByClause {$clauses.Insert("sampleByClause");}
		| {!$clauses.Has("ttlClause")}? ttlClause {$clauses.Insert("ttlClause");}
		| {!$clauses.Has("settingsClause")}? settingsClause {$clauses.Insert("settingsClause");}
	)*;
...
engineExpr:
	ENGINE EQ_SINGLE? identifierOrNull (
...
dropStmt: (DETACH | DROP) DATABASE (IF EXISTS)? databaseIdentifier clusterClause? # DropDatabaseStmt
	| (DETACH | DROP) (DICTIONARY | TEMPORARY? TABLE | VIEW) (
		IF EXISTS
	)? tableIdentifier clusterClause? (NO DELAY)? # DropTableStmt;

// DELETE statement
deleteStmt: DELETE FROM tableExpr clusterClause? whereClause;
...
insertStmt:
	...
	) columnsClause? settingsClause? dataClause;
...
dataClause:
	...
	| ctes? selectUnionStmt SEMICOLON? EOF	# DataClauseSelect;

// KILL statement

killStmt:
	KILL MUTATION clusterClause? whereClause (
		SYNC
		| ASYNC
		| TEST
	)? # KillMutationStmt;
...
renameStmt:
	RENAME TABLE tableIdentifier TO tableIdentifier (
		COMMA tableIdentifier TO tableIdentifier
	)* clusterClause?;
...
selectUnionStmt:
	selectStmtWithParens (
		UNION (ALL | DISTINCT) selectStmtWithParens
	)*;
selectStmtWithParens:
	selectStmt
	| LPAREN selectUnionStmt RPAREN;
selectStmt:
	withClause? SELECT DISTINCT? topClause? columnExprList fromClause? arrayJoinClause? windowClause
		? prewhereClause? whereClause? groupByClause? (
		WITH (CUBE | ROLLUP)
...
groupByClause:
	GROUP BY (
		...
		| columnExprList
		| GROUPING SETS (
			LPAREN columnExprList (
				COMMA (columnExprList | emptyListExpr)
			)* RPAREN
		)
	);
...
joinOpCross: (GLOBAL | LOCAL)? CROSS JOIN | COMMA;
...
orderExpr:
	columnExpr (ASCENDING | DESCENDING | DESC)? (
		NULLS (FIRST | LAST)
...
showStmt:
	...
	| SHOW TEMPORARY? TABLES ((FROM | IN) databaseIdentifier)? (
		LIKE STRING_LITERAL
		| whereClause
	)? limitClause? # showTablesStmt;
...
columnTypeExpr:
	...
	| identifier LPAREN identifier columnTypeExpr (
		COMMA identifier columnTypeExpr
	)* RPAREN															# ColumnTypeExprNested // Nested
...
columnsExpr: (tableIdentifier DOT)? ASTERISK (
...
columnExpr:
	CASE columnExpr? (WHEN columnExpr THEN columnExpr)+ (
		ELSE columnExpr
	)? END												# ColumnExprCase
	...
	| SUBSTRING LPAREN columnExpr FROM columnExpr (
		FOR columnExpr
...
emptyListExpr: LPAREN RPAREN;

// Tables
tableExpr:
	tableIdentifier							# TableExprIdentifier
	| tableFunctionExpr						# TableExprFunction
	| LPAREN ctes? selectUnionStmt RPAREN	# TableExprSubquery
...
identifierOrNull:
	identifier
	| NULL_SQL; // NULL_SQL can be only 'Null' here.
